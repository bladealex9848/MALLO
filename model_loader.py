...
def load_models_from_openrouter() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if not response.ok:
            logging.error(f"Error al cargar modelos desde OpenRouter API: {response.status_code} {response.reason}")
            return []
        ...
        if not data.get("data") or not isinstance(data["data"], list):
            logging.warning("No se encontraron modelos en la respuesta de OpenRouter API")
            return []
        ...
        for model in data["data"]:
            ...
            is_free = ":free" in model.get("id", "") or (model.get("context_length_free", 0) > 0)
            ...
            if is_free:
                api_models.append({
                    ...
                    "provider": model.get("id", "").split("/")[0] if "/" in model.get("id", "") else "Desconocido",
...
def load_models_from_groq() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Verificar si hay error
        if not response.ok:
            logging.error(f"Error al cargar modelos desde Groq API: {response.status_code} {response.reason}")
            return []
...
def load_models_from_ollama() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Verificar si hay error
        if not response.ok:
            logging.error(f"Error al cargar modelos desde Ollama API: {response.status_code} {response.reason}")
            return []
        ...
        for model in data["models"]:
            api_models.append({
                ...
                "capabilities": get_ollama_model_capabilities(model.get("name", "")),
